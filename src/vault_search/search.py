...
class SearchEngine:
    ...
    def search(
        ...
        # 执行 FTS 搜索
        fts_results = self.db.search_fts(query, limit, area, tags)
        
        # 判断是否需要 fallback（CJK 查询或无结果）
        if self._needs_fallback(query, fts_results):
            like_results = self.db.search_like(query, limit, area, tags)
        else:
            like_results = []
        ...
        # 构建 SearchResult 对象
        return [self._build_result(item, query) for item in merged]
    ...
    def _build_result(self, item: dict, query: str) -> SearchResult:
        """从数据库结果构建 SearchResult 对象"""
        # 获取标签
        tags = self.db.get_tags(item["path"])
